...
func TestClassicWoWPvPLeaderboards(t *testing.T) {
	dat, _, err := usClient.ClassicWoWPvPLeaderboards(context.Background(), 1, 4, wowgd.Bracket2v2)
	if err != nil {
		fmt.Println(err)
		t.Fail()
	}
	...
	dat, _, err = usClient.ClassicWoWPvPLeaderboards(context.Background(), 1, 4, wowgd.Bracket3v3)
	if err != nil {
		fmt.Println(err)
		t.Fail()
	}
