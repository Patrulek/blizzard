...
import (
	...
	"github.com/Patrulek/blizzard/v3/wowgd"
)
...
func TestWoWCharacterPvPBracketStatistics(t *testing.T) {
	dat, _, err := usClient.WoWCharacterPvPBracketStatistics(context.Background(), "tichondrius", "tmpikaboo", wowgd.Bracket2v2)
	if err != nil {
		fmt.Println(err)
		t.Fail()
	}
	...
	dat, _, err = usClient.WoWCharacterPvPBracketStatistics(context.Background(), "tichondrius", "tmpikaboo", wowgd.Bracket3v3)
	if err != nil {
		fmt.Println(err)
		t.Fail()
	}
	...
	dat, _, err = usClient.WoWCharacterPvPBracketStatistics(context.Background(), "icecrown", "soballer", wowgd.BracketRBG)
	if err != nil {
		fmt.Println(err)
		t.Fail()
	}
